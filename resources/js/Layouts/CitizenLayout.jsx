...
export default function CitizenLayout({ children }) {
    ...
    const navLinks = [
        { name: 'Dashboard', href: route('citizen.dashboard'), icon: 'dashboard', active: route().current('citizen.dashboard') },
        { name: 'Land Records', href: route('citizen.land-records.index'), icon: 'domain', active: route().current('citizen.land-records.*') },
        { name: 'Payments', href: route('citizen.payments'), icon: 'payments', active: route().current('citizen.payments') },
        ...
        <div className="bg-surface text-on-surface min-h-screen">
            ...
            <header className="bg-[#002D62] dark:bg-slate-900 text-white font-public-sans font-bold text-lg fixed top-0 left-0 right-0 z-50 flex justify-between items-center w-full px-4 h-14 border-b border-white/10 shadow-sm transition-colors duration-200">
                <div className="flex items-center gap-3">
                    ...
                    <Link href={route('dashboard')} className="text-white font-bold tracking-tight mr-4">Land Records Portal</Link>
                    ...
                    {auth.user ? (
                        <>
                            ...
                            <div className="w-8 h-8 rounded-full bg-primary-fixed-dim flex items-center justify-center overflow-hidden border border-white/20">
                                <img alt="User Profile" className="w-full h-full object-cover" src="https://lh3.googleusercontent.com/aida-public/AB6AXuD5OD1uJVf78jLxybbn2YhRClXFzSGz4k2LO7rXiEOP8SbNMWh5hSQfsUMm5RKwnX024OSl_hu8kv-UeLsgqn6OXNH1ABrmRPjP8lT62fsj2x2Us3FyX0TGb6_MXzxBaDAK08yC9QRXZiKs8UuD711oz33-iU6yajdeYoB77RgkNJo6EanzwD27ftz1p31A-Qj9_KemtnXA-iBOuZ4yK46aYnUz0V35-fUYtIbL0q8JODNYcZh9s4vAaImAOODudEwIb9pnogccYw"/>
                            </div>
                        </>
                    ) : (
                        <>
                            ...
                            <Link href={route('register')} className="font-label-md text-white bg-white/10 border border-white/30 px-3 py-1.5 rounded-lg hover:bg-white/20 transition-colors">
                                Register
                            </Link>
                        </>
                    )}
